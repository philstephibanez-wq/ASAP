(function () {
  ...
  if (window.__ASAP_RUNTIME_ROBOT_INSTALLED__ === true) {
    return;
  }

  window.__ASAP_RUNTIME_ROBOT_INSTALLED__ = true;

  function inspectPage() {
    var bodyText = document.body ? document.body.innerText || '' : '';
    var runtimeError = /ASAP_[A-Z0-9_]*ERROR|Fatal error|Parse error|Warning:/i.test(bodyText);
  ...
  chrome.runtime.onMessage.addListener(function (message, sender, sendResponse) {
    if (!message || message.type !== 'ASAP_RUNTIME_ROBOT_INSPECT') {
      return false;
    }
