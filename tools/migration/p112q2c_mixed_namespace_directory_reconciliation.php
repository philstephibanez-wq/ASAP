...
function normalizeDirectoryCase(string $parent, string $from, string $to): void
{
    ...
    $tmp = '__P112Q2C_CASE_TMP_' . $from . '_' . bin2hex(random_bytes(4));
...
function shouldSkipPath(string $path): bool
{
    ...
    if (str_contains(strtolower($normalized), '/tools/migration/p112q2c_')) {
        return true;
    }

    if (str_contains($normalized, '/DOC/P112Q2C_')) {
        return true;
    }

    if (str_contains($normalized, '/content/markdown/mixed-namespace-directory-reconciliation.md')) {
        return true;
    }
