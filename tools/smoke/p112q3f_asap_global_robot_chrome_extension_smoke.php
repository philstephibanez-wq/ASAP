...
/**
 * P112Q3F ASAP global robot + Chrome extension smoke.
 *
 * Public smoke test.
 * Contract:
 *   - the ASAP global regression recipe includes this smoke;
...
$required = [
    ...
    'tools/chrome_extension/asap_runtime_robot/manifest.json',
    'tools/chrome_extension/asap_runtime_robot/content.js',
    'tools/chrome_extension/asap_runtime_robot/popup.html',
    'tools/chrome_extension/asap_runtime_robot/popup.css',
    'tools/chrome_extension/asap_runtime_robot/popup.js',
    'tools/chrome_extension/asap_runtime_robot/README.md',
    'DOC/patches/P112Q3F_ASAP_GLOBAL_ROBOTIZED_RECIPE_CHROME_EXTENSION/PATCH.md',
    'DOC/patches/P112Q3F_ASAP_GLOBAL_ROBOTIZED_RECIPE_CHROME_EXTENSION/CHANGELOG.md',
];
...
$globalRecipe = readFileOrFail($root, 'tools/recipes/asap_global_regression_recipe.php');
if (!str_contains($globalRecipe, 'P112Q3F_SMOKE') || !str_contains($globalRecipe, 'p112q3f_asap_global_robot_chrome_extension_smoke.php')) {
    fwrite(STDERR, 'P112Q3F_SMOKE_FAILED: GLOBAL_RECIPE_STEP_MISSING' . PHP_EOL);
    exit(1);
}

$robotRecipe = readFileOrFail($root, 'tools/recipes/p112q3f_asap_global_robotized_recipe.php');
if (!str_contains($robotRecipe, 'P112Q3F_ASAP_GLOBAL_ROBOTIZED_RECIPE_OK') || !str_contains($robotRecipe, 'asap_global_regression_recipe.php')) {
    fwrite(STDERR, 'P112Q3F_SMOKE_FAILED: ROBOTIZED_RECIPE_CONTRACT_MISSING' . PHP_EOL);
    exit(1);
}

$manifestPath = pathFromRelative($root, 'tools/chrome_extension/asap_runtime_robot/manifest.json');
...
$content = readFileOrFail($root, 'tools/chrome_extension/asap_runtime_robot/content.js');
if (!str_contains($content, 'ASAP_RUNTIME_ROBOT_INSPECT') || str_contains($content, 'fetch(') || str_contains($content, 'XMLHttpRequest')) {
    fwrite(STDERR, 'P112Q3F_SMOKE_FAILED: CONTENT_SCRIPT_CONTRACT_INVALID' . PHP_EOL);
    exit(1);
}

$popup = readFileOrFail($root, 'tools/chrome_extension/asap_runtime_robot/popup.js');
if (!str_contains($popup, 'ASAP_RUNTIME_ROBOT_INSPECT') || !str_contains($popup, 'chrome.tabs.query')) {
    fwrite(STDERR, 'P112Q3F_SMOKE_FAILED: POPUP_CONTRACT_INVALID' . PHP_EOL);
    exit(1);
}

echo 'P112Q3F_ASAP_GLOBAL_ROBOT_CHROME_EXTENSION_SMOKE_OK' . PHP_EOL;
exit(0);
