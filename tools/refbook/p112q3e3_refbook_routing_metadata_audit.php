...
/**
 * P112Q3E3 ROUTING RefBook metadata audit.
 *
 * Public CLI tool.
 * Role:
 *   Generate an observable JSON/MD/HTML report proving the ASAP ROUTING domain
 *   is fully covered by Reflection-backed RefBook metadata.
 */
$root = dirname(__DIR__, 2);
$strict = in_array('--strict', $argv, true) || getenv('ASAP_P112Q3E3_STRICT') === '1';
...
$reportRoot = $root . DIRECTORY_SEPARATOR . 'var' . DIRECTORY_SEPARATOR . 'reports' . DIRECTORY_SEPARATOR . 'p112q3e3_refbook_routing_metadata';
...
$snapshot = $builder->build($result, $routingRoot);

$timestamp = date('Ymd_His');
$base = $reportRoot . DIRECTORY_SEPARATOR . 'P112Q3E3_REFBOOK_ROUTING_METADATA_' . $timestamp;
$jsonPath = $base . '.json';
$mdPath = $base . '.md';
$htmlPath = $base . '.html';
$snapshotPath = $snapshotRoot . DIRECTORY_SEPARATOR . 'snapshot.routing.latest.json';
...
$summary = $validation['summary'];
echo 'P112Q3E3_REFBOOK_ROUTING_METADATA_AUDIT_OK' . PHP_EOL;
...
if ($strict && $summary['violations'] > 0) {
    fwrite(STDERR, 'P112Q3E3_REFBOOK_ROUTING_METADATA_STRICT_FAILED: Violations=' . (string) $summary['violations'] . PHP_EOL);
    exit(2);
}

if ($strict) {
    echo 'P112Q3E3_REFBOOK_ROUTING_METADATA_STRICT_OK' . PHP_EOL;
}
exit(0);
...
function requireRefBookCore(string $root): void
{
    ...
    foreach ($files as $relative) {
        ...
        if (!is_file($path)) {
            fwrite(STDERR, 'P112Q3E3_REFBOOK_CORE_FILE_MISSING: ' . $path . PHP_EOL);
            exit(1);
        }
        require_once $path;
    }
...
function ensureDirectory(string $path): void
{
    ...
    if (!mkdir($path, 0775, true) && !is_dir($path)) {
        fwrite(STDERR, 'P112Q3E3_DIRECTORY_CREATE_FAILED: ' . $path . PHP_EOL);
        exit(1);
    }
}
...
function writeJson(string $path, array $data): void
{
    ...
    if (!is_string($json)) {
        fwrite(STDERR, 'P112Q3E3_JSON_ENCODE_FAILED: ' . $path . PHP_EOL);
        exit(1);
    }
...
function writeText(string $path, string $content): void
{
    if (file_put_contents($path, $content) === false) {
        fwrite(STDERR, 'P112Q3E3_FILE_WRITE_FAILED: ' . $path . PHP_EOL);
        exit(1);
    }
}
...
function buildMarkdown(array $report): string
{
    ...
    $lines = [
        '# P112Q3E3 ROUTING RefBook Metadata Report',
        '',
        '- Palier: P112Q3E3',
        '- Domain: ROUTING',
...
{
    return '<!doctype html><html lang="fr"><head><meta charset="utf-8"><title>P112Q3E3 ROUTING RefBook Metadata</title></head><body><pre>'
