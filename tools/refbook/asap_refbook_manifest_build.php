...
if (!is_dir($frameworkRoot)) {
    fwrite(STDERR, 'ASAP_REFBOOK_MANIFEST_FRAMEWORK_ROOT_NOT_FOUND=' . $frameworkRoot . PHP_EOL);
    exit(1);
}

if (!is_dir($outputRoot) && !mkdir($outputRoot, 0775, true) && !is_dir($outputRoot)) {
    fwrite(STDERR, 'ASAP_REFBOOK_MANIFEST_OUTPUT_ROOT_CREATE_FAILED=' . $outputRoot . PHP_EOL);
    exit(1);
}
...
if ($json === false) {
    fwrite(STDERR, 'ASAP_REFBOOK_MANIFEST_JSON_ENCODE_FAILED' . PHP_EOL);
    exit(1);
}
...
echo 'ASAP_REFBOOK_MANIFEST_SCHEMA=ASAP_REFBOOK_SOURCE_MANIFEST_V1' . PHP_EOL;
echo 'ASAP_REFBOOK_MANIFEST_DOMAINS=' . count($domains) . PHP_EOL;
echo 'ASAP_REFBOOK_MANIFEST_SYMBOLS=' . count($symbols) . PHP_EOL;
echo 'ASAP_REFBOOK_MANIFEST_OUTPUT=' . $output . PHP_EOL;
exit(0);
