...
/*
 * P112Q2T — Apply ASAP_REFBOOK baseline tags to every ASAP framework symbol.
 *
 * Safety:
 ...
 * - Existing ASAP_REFBOOK tags are preserved.
 */
...
$asapRoot = require_dir($asapRoot, 'P112Q2T_ASAP_ROOT_NOT_FOUND');
$frameworkRoot = require_dir($asapRoot . DIRECTORY_SEPARATOR . 'framework' . DIRECTORY_SEPARATOR . 'Asap', 'P112Q2T_FRAMEWORK_ROOT_NOT_FOUND');
...
foreach ($files as $file) {
    ...
    if (str_contains($content, 'ASAP_REFBOOK:')) {
        $skipped++;
        continue;
    }
...
function buildTagBlock(array $symbol, string $domain): string
{
    ...
    $lines = [
        ...
        ' * ASAP_REFBOOK:',
        ' *   domain: ' . $domain,
        ' *   role: ' . ucfirst($humanKind) . ' ' . $name . ' belongs to the ' . $domain . ' ASAP framework domain.',
        ' *   contract:',
...
MD;
    ...
    writeFile($asapRoot . DIRECTORY_SEPARATOR . 'DOC' . DIRECTORY_SEPARATOR . 'P112Q2T_ASAP_REFBOOK_ALL_SOURCE_TAGS_BASELINE.md', $doc);
}
