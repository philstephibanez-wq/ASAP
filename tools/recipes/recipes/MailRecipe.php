...
final class MailRecipe implements RecipeInterface
{
    ...
    {
        $mail = new \ASAP\Mail\Mail('recipe@example.org', 'ASAP recipe mail', 'ASAP_MAIL_BODY_OK');
        $context->assert($mail->to === 'recipe@example.org', 'ASAP_MAIL_TO_INVALID_AFTER_CONSTRUCT');
        $context->assert($mail->subject === 'ASAP recipe mail', 'ASAP_MAIL_SUBJECT_INVALID_AFTER_CONSTRUCT');
        $context->assert(str_contains($mail->body, 'ASAP_MAIL_BODY_OK'), 'ASAP_MAIL_BODY_INVALID_AFTER_CONSTRUCT');

        try {
            new \ASAP\Mail\Mail('invalid-address', 'bad', 'bad');
            $context->assert(false, 'ASAP_MAIL_INVALID_ADDRESS_DID_NOT_FAIL');
        } catch (\InvalidArgumentException) {
        }

        try {
            (new \ASAP\Mail\PhpMailer())->send($mail);
            $context->assert(false, 'ASAP_MAIL_UNCONFIGURED_TRANSPORT_DID_NOT_FAIL');
        } catch (\RuntimeException $exception) {
            $context->assert($exception->getMessage() === 'ASAP_PHPMAILER_RUNTIME_NOT_CONFIGURED', 'ASAP_MAIL_TRANSPORT_FAILURE_CODE_INVALID', $exception->getMessage());
        }

        $subject = 'ASAP Mailpit Recipe ' . $context->runId();
        $body = 'ASAP_MAILPIT_BODY_OK ' . $context->runId();
        ...
        $context->assert(str_contains($raw, 'robot@example.org'), 'ASAP_MAILPIT_TO_MISSING', $raw);
        $context->assert(str_contains($raw, $subject), 'ASAP_MAILPIT_SUBJECT_MISSING', $raw);
        $context->assert(str_contains($raw, $body), 'ASAP_MAILPIT_BODY_MISSING', $raw);

        return ['ASAP_MAILPIT_AVAILABLE_OK', 'ASAP_MAIL_SEND_OK', 'ASAP_MAIL_RECEIVED_OK', 'ASAP_MAIL_CONTENT_OK', 'ASAP_MAIL_OK'];
    }
    ...
    {
        return (string)(getenv('ASAP_RECIPE_MAILPIT_SMTP_HOST') ?: '127.0.0.1');
    }

    private function mailpitSmtpPort(): int
    {
        return (int)((string)(getenv('ASAP_RECIPE_MAILPIT_SMTP_PORT') ?: '1025'));
    }
    ...
    private function assertMailpitAvailable(): void
    {
        ...
        if (!is_string($messages)) {
            throw RecipeAssertionFailedException::because('ASAP_MAILPIT_HTTP_API_UNAVAILABLE', $this->mailpitHttpBase() . '/api/v1/messages');
        }
        $socket = @fsockopen($this->mailpitSmtpHost(), $this->mailpitSmtpPort(), $errno, $errstr, 3.0);
        if (!is_resource($socket)) {
            throw RecipeAssertionFailedException::because('ASAP_MAILPIT_SMTP_UNAVAILABLE', $this->mailpitSmtpHost() . ':' . $this->mailpitSmtpPort() . ' :: ' . $errstr);
        }
        fclose($socket);
    }
    ...
    private function sendSmtp(string $to, string $subject, string $body): void
    {
        ...
        if (!is_resource($socket)) {
            throw RecipeAssertionFailedException::because('ASAP_MAILPIT_SMTP_CONNECT_FAILED', $errstr);
        }
        stream_set_timeout($socket, 5);
        $this->expect($socket, '220', 'BANNER');
        $this->command($socket, 'HELO asap-recipe.local', '250', 'HELO');
        $this->command($socket, 'MAIL FROM:<asap-recipe@example.org>', '250', 'MAIL_FROM');
    ...
    private function expect($socket, string $expected, string $step): void
    {
        ...
        if (!is_string($line) || !str_starts_with($line, $expected)) {
            throw RecipeAssertionFailedException::because('ASAP_MAILPIT_SMTP_UNEXPECTED_' . $step, trim((string)$line));
        }
    }
    ...
    private function waitForMailpitSubject(string $subject, float $seconds): array
    {
        ...
        throw RecipeAssertionFailedException::because('ASAP_MAIL_NOT_RECEIVED_FAIL', $subject);
    }
    ...
    private function httpJson(string $url): array
    {
        ...
        if (!is_string($body)) {
            throw RecipeAssertionFailedException::because('ASAP_MAILPIT_HTTP_READ_FAILED', $url);
        }
        $decoded = json_decode($body, true);
        if (!is_array($decoded)) {
            throw RecipeAssertionFailedException::because('ASAP_MAILPIT_HTTP_JSON_INVALID', $url);
        }
        return $decoded;
    }
}
