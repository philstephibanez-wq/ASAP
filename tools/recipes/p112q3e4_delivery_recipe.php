...
/**
 * P112Q3E4 delivery recipe.
 *
 * Public CLI delivery gate.
 */
$root = dirname(__DIR__, 2);
$reportRoot = $root . DIRECTORY_SEPARATOR . 'var' . DIRECTORY_SEPARATOR . 'reports' . DIRECTORY_SEPARATOR . 'p112q3e4_delivery_recipe';
...
$steps = [
    ...
    ['id' => 'P112Q3E4_AUDIT_STRICT', 'command' => ['php', 'tools/refbook/p112q3e4_refbook_http_metadata_audit.php', '--strict']],
...
foreach ($steps as $step) {
    ...
    if ($result['output'] !== '') { echo $result['output'] . PHP_EOL; }
    if ($result['status'] !== 'OK') { $failed = true; }
}

$summary = ['palier' => 'P112Q3E4_DELIVERY_RECIPE', 'total_steps' => count($results), 'failed_steps' => count(array_filter($results, static function (array $result): bool { return $result['status'] !== 'OK'; })), 'status' => $failed ? 'FAILED' : 'OK'];
$report = ['summary' => $summary, 'steps' => $results];
$timestamp = date('Ymd_His');
$base = $reportRoot . DIRECTORY_SEPARATOR . 'P112Q3E4_DELIVERY_RECIPE_' . $timestamp;
...
if ($failed) { fwrite(STDERR, 'P112Q3E4_DELIVERY_RECIPE_FAILED' . PHP_EOL); exit(1); }
echo 'P112Q3E4_DELIVERY_RECIPE_OK' . PHP_EOL;
exit(0);
...
{
    $relative = $command[1] ?? '';
    if ($relative === '' || !is_file($root . DIRECTORY_SEPARATOR . str_replace('/', DIRECTORY_SEPARATOR, $relative))) { return ['id' => $id, 'status' => 'FAILED', 'exit_code' => 127, 'command' => implode(' ', $command), 'output' => 'REQUIRED_STEP_FILE_MISSING: ' . $relative]; }
    $parts = [];
...
function ensureDirectory(string $path): void
{
    ...
    if (!mkdir($path, 0775, true) && !is_dir($path)) { fwrite(STDERR, 'P112Q3E4_DELIVERY_RECIPE_DIRECTORY_FAILED: ' . $path . PHP_EOL); exit(1); }
}
...
function writeJson(string $path, array $data): void
{
    ...
    if (!is_string($json)) { fwrite(STDERR, 'P112Q3E4_DELIVERY_RECIPE_JSON_FAILED: ' . $path . PHP_EOL); exit(1); }
    writeText($path, $json . PHP_EOL);
}

function writeText(string $path, string $content): void
{
    if (file_put_contents($path, $content) === false) { fwrite(STDERR, 'P112Q3E4_DELIVERY_RECIPE_WRITE_FAILED: ' . $path . PHP_EOL); exit(1); }
}

/** @param array<string,mixed> $report */
function buildMarkdown(array $report): string
{
    $lines = ['# P112Q3E4 Delivery Recipe', '', 'Status: **' . $report['summary']['status'] . '**', '', '## Steps', ''];
...
{
    return '<!doctype html><html lang="fr"><head><meta charset="utf-8"><title>P112Q3E4 Delivery Recipe</title></head><body><pre>'
