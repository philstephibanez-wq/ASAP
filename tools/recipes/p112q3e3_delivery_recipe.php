...
$reportRoot = $root . DIRECTORY_SEPARATOR . 'var' . DIRECTORY_SEPARATOR . 'reports' . DIRECTORY_SEPARATOR . 'p112q3e3_delivery_recipe';
...
$steps = [
    ...
    ['id' => 'P112Q3E3_AUDIT_STRICT', 'command' => ['php', 'tools/refbook/p112q3e3_refbook_routing_metadata_audit.php', '--strict']],
...
$summary = [
    'palier' => 'P112Q3E3_DELIVERY_RECIPE',
...
$base = $reportRoot . DIRECTORY_SEPARATOR . 'P112Q3E3_DELIVERY_RECIPE_' . $timestamp;
...
if ($failed) {
    fwrite(STDERR, 'P112Q3E3_DELIVERY_RECIPE_FAILED' . PHP_EOL);
    exit(1);
}

echo 'P112Q3E3_DELIVERY_RECIPE_OK' . PHP_EOL;
exit(0);
...
function ensureDirectory(string $path): void
{
    ...
    if (!mkdir($path, 0775, true) && !is_dir($path)) {
        fwrite(STDERR, 'P112Q3E3_DELIVERY_RECIPE_DIRECTORY_FAILED: ' . $path . PHP_EOL);
        exit(1);
    }
}
...
function writeJson(string $path, array $data): void
{
    ...
    if (!is_string($json)) {
        fwrite(STDERR, 'P112Q3E3_DELIVERY_RECIPE_JSON_FAILED: ' . $path . PHP_EOL);
        exit(1);
    }
...
function writeText(string $path, string $content): void
{
    if (file_put_contents($path, $content) === false) {
        fwrite(STDERR, 'P112Q3E3_DELIVERY_RECIPE_WRITE_FAILED: ' . $path . PHP_EOL);
        exit(1);
    }
}
...
{
    $lines = ['# P112Q3E3 Delivery Recipe', '', 'Status: **' . $report['summary']['status'] . '**', '', '## Steps', ''];
...
{
    return '<!doctype html><html lang="fr"><head><meta charset="utf-8"><title>P112Q3E3 Delivery Recipe</title></head><body><pre>'
