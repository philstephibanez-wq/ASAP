...
/**
 * P112Q3F ASAP global robotized recipe.
 *
 * Public CLI recipe.
 * Role:
 *   Run the existing ASAP global regression gate, then verify the local
...
$reportRoot = $root . DIRECTORY_SEPARATOR . 'var' . DIRECTORY_SEPARATOR . 'reports' . DIRECTORY_SEPARATOR . 'p112q3f_asap_global_robotized_recipe';
ensureDirectory($reportRoot);

$steps = [
    ['id' => 'ASAP_GLOBAL_REGRESSION_RECIPE', 'command' => ['php', 'tools/recipes/asap_global_regression_recipe.php']],
    ['id' => 'P112Q3F_CHROME_EXTENSION_SMOKE', 'command' => ['php', 'tools/smoke/p112q3f_asap_global_robot_chrome_extension_smoke.php']],
];
...
$summary = [
    'palier' => 'P112Q3F_ASAP_GLOBAL_ROBOTIZED_RECIPE',
...
$base = $reportRoot . DIRECTORY_SEPARATOR . 'P112Q3F_ASAP_GLOBAL_ROBOTIZED_RECIPE_' . $timestamp;
...
if ($failed) {
    fwrite(STDERR, 'P112Q3F_ASAP_GLOBAL_ROBOTIZED_RECIPE_FAILED' . PHP_EOL);
    exit(1);
}

echo 'P112Q3F_ASAP_GLOBAL_ROBOTIZED_RECIPE_OK' . PHP_EOL;
exit(0);
...
{
    $manifestPath = $root . DIRECTORY_SEPARATOR . 'tools' . DIRECTORY_SEPARATOR . 'chrome_extension' . DIRECTORY_SEPARATOR . 'asap_runtime_robot' . DIRECTORY_SEPARATOR . 'manifest.json';
    $manifest = json_decode((string) file_get_contents($manifestPath), true);

    return [
        'path' => 'tools/chrome_extension/asap_runtime_robot',
...
{
    $lines = ['# P112Q3F ASAP Global Robotized Recipe', '', 'Status: **' . $report['summary']['status'] . '**', '', '## Steps', ''];
...
{
    return '<!doctype html><html lang="fr"><head><meta charset="utf-8"><title>P112Q3F ASAP Global Robotized Recipe</title></head><body><pre>'
