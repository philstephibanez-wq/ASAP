...
/**
 * P113D1 ASAP RefBook REST API robotized recipe.
 *
 * Role:
 *   Generate observable JSON/MD/HTML reports proving that ASAP exposes its
...
$report = [
    'palier' => 'P113D1_ASAP_REFBOOK_REST_API',
...
if ($failed !== []) {
    fail('P113D1_ASAP_REFBOOK_REST_API_RECIPE_FAILED');
}

echo 'P113D1_ASAP_REFBOOK_REST_API_RECIPE_OK' . PHP_EOL;
exit(0);

function checkEndpoint(ASAP\RefBook\Api\RefBookRestApi $api, string $path, string $method, int $expectedStatus, string $id): array
{
...
{
    $lines = ['# P113D1 — ASAP RefBook REST API', '', 'Status: **' . $report['status'] . '**', '', '## Checks', ''];
    foreach ($report['checks'] as $check) {
        $lines[] = '- ' . ($check['ok'] ? 'OK' : 'FAIL') . ' — ' . $check['method'] . ' ' . $check['path'] . ' — HTTP ' . $check['observed_status'];
    }
    $lines[] = '';
    $lines[] = '## Assets';
...
function htmlReport(array $report): string
{
    return '<!doctype html><html lang="fr"><head><meta charset="utf-8"><title>P113D1 RefBook REST API</title>'
        ...
        . '</head><body><h1>P113D1 — ASAP RefBook REST API</h1><p>Status: <strong class="' . ($report['status'] === 'OK' ? 'ok' : 'fail') . '">' . htmlspecialchars($report['status'], ENT_QUOTES, 'UTF-8') . '</strong></p><pre>'
