 ...
 *
 * Role:
 *   Load one explicit ASAP site security policy from XML.
 *
 * Responsibility:
...
final class SiteSecurityPolicyLoader
{
    ...
    public function load(string $securityFile): SiteSecurityPolicy
    {
        if (!is_file($securityFile)) {
            throw ContractException::because('ASAP_SITE_SECURITY_FILE_MISSING', $securityFile);
        }

        $xml = simplexml_load_file($securityFile);

        if (!$xml instanceof SimpleXMLElement) {
            throw ContractException::because('ASAP_SITE_SECURITY_XML_INVALID', $securityFile);
        }

        if (!isset($xml->fsm)) {
            throw ContractException::because('ASAP_SITE_SECURITY_FSM_MISSING', $securityFile);
        }

        if (!isset($xml->acl)) {
            throw ContractException::because('ASAP_SITE_SECURITY_ACL_MISSING', $securityFile);
        }
