 ...
 * OPUS_REFBOOK:
 *   domain: URL
 *   role: Class Url belongs to the URL Opus framework domain.
 *   contract:
...
final class Url
{
    ...
    public function __construct(string $basePath = '')
    {
        ...
        if ($basePath !== '' && preg_match('/^[a-z][a-z0-9+.-]*:\/\//i', $basePath) === 1) {
            ...
            if (!is_array($parts)) {
                throw new \InvalidArgumentException('OPUS_URL_PARSE_FAILED');
            }
    ...
    {
        if (trim($slug) === '') {
            throw new \InvalidArgumentException('OPUS_URL_ROUTE_SLUG_EMPTY');
        }
    ...
    public function setProtocol(string $protocol): self
    {
        if ($protocol !== '' && preg_match('/^[a-z][a-z0-9+.-]*$/i', $protocol) !== 1) {
            throw new \InvalidArgumentException('OPUS_URL_PROTOCOL_INVALID');
        }
    ...
    {
        if ($path !== '' && $path[0] !== '/') {
            throw new \InvalidArgumentException('OPUS_URL_LEGACY_PATH_INVALID');
        }
    ...
    {
        if (trim($path) === '') {
            throw new \InvalidArgumentException('OPUS_URL_PATH_EMPTY');
        }
