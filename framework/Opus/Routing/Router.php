 ...
 * OPUS_REFBOOK:
 *   domain: ROUTING
 *   role: Class Router belongs to the ROUTING Opus framework domain.
 *   contract:
 ...
 *
 * Role:
 *   Match a normalized request against explicit Opus routes.
 *
 * Responsibility:
...
final class Router implements RefBookInspectableInterface
{
    ...
    {
        if ($this->routes === []) {
            throw ContractException::because('OPUS_ROUTE_TABLE_EMPTY');
        }
    }
    ...
    #[OpusRefBookMethod(
        ...
        errors: ['OPUS_ROUTES_FILE_MISSING', 'OPUS_ROUTES_XML_INVALID', 'OPUS_ROUTE_TARGET_MISSING', 'OPUS_ROUTE_DEFAULT_PARAM_NAME_EMPTY', 'OPUS_ROUTE_ACL_METADATA_INVALID', 'OPUS_ROUTE_FSM_METADATA_INVALID'],
        testRefs: ['tests/Contract/RefBookRoutingMetadataContractTest.php'],
        examples: ['routing-overview', 'secure-dispatch-gate'],
        diagrams: ['routing-runtime', 'secure-dispatch-runtime'],
        introducedIn: 'P112Q3E3'
    ...
    public static function fromXml(string $routesFile): self
    {
        if (!is_file($routesFile)) {
            throw ContractException::because('OPUS_ROUTES_FILE_MISSING', $routesFile);
        }

        $xml = simplexml_load_file($routesFile);

        if (!$xml instanceof SimpleXMLElement) {
            throw ContractException::because('OPUS_ROUTES_XML_INVALID', $routesFile);
        }
        ...
        foreach ($xml->route as $routeNode) {
            ...
            if (!$target instanceof SimpleXMLElement) {
                throw ContractException::because('OPUS_ROUTE_TARGET_MISSING', (string) ($routeNode['name'] ?? ''));
            }
            ...
            if (isset($routeNode->defaults)) {
                foreach ($routeNode->defaults->param as $param) {
                    ...
                    if ($paramName === '') {
                        throw ContractException::because('OPUS_ROUTE_DEFAULT_PARAM_NAME_EMPTY', (string) ($routeNode['name'] ?? ''));
                    }
    ...
    #[OpusRefBookMethod(
        ...
        sideEffects: ['none'],
        errors: ['OPUS_REQUEST_OUTSIDE_SITE_BASE_PATH', 'OPUS_ROUTE_METHOD_NOT_ALLOWED', 'OPUS_ROUTE_NOT_FOUND'],
        testRefs: ['tests/Contract/RefBookRoutingMetadataContractTest.php'],
        examples: ['routing-overview', 'secure-dispatch-gate'],
        diagrams: ['routing-runtime', 'secure-dispatch-runtime'],
        introducedIn: 'P112Q3E3'
    ...
    public function match(Request $request, SiteDefinition $site): RouteMatch
    {
        ...
        throw ContractException::because('OPUS_ROUTE_NOT_FOUND', $localPath);
    }
    ...
    private function toLocalPath(string $path, string $basePath): string
    {
        ...
        if (!str_starts_with($path, $basePath . '/')) {
            throw ContractException::because('OPUS_REQUEST_OUTSIDE_SITE_BASE_PATH', $path);
        }
    ...
    private static function parseMethods(?string $rawMethods): array
    {
        ...
        if ($methods === []) {
            throw ContractException::because('OPUS_ROUTE_METHODS_EMPTY');
        }
    ...
    private static function parsePriority(?string $rawPriority): int
    {
        ...
        if (preg_match('/^-?\d+$/', $rawPriority) !== 1) {
            throw ContractException::because('OPUS_ROUTE_PRIORITY_INVALID', $rawPriority);
        }
    ...
    private static function readAclMetadata(SimpleXMLElement $routeNode): ?string
    {
        ...
        if ($resource === null || $privilege === null) {
            throw ContractException::because('OPUS_ROUTE_ACL_METADATA_INVALID', (string) ($routeNode['name'] ?? ''));
        }
    ...
    private static function readFsmGuardMetadata(SimpleXMLElement $routeNode): ?string
    {
        ...
        if ($signal === null) {
            throw ContractException::because('OPUS_ROUTE_FSM_METADATA_INVALID', (string) ($routeNode['name'] ?? ''));
        }
