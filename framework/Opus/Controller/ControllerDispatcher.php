<?php

declare(strict_types=1);

namespace Opus\Controller;

use Opus\Application\ApplicationPaths;
use Opus\Http\Request;
use Opus\Http\Response;
use Opus\Renderer\HtmlRenderer;
use Opus\Renderer\ViewModel;
use Opus\Routing\RouteMatch;
use Opus\Template\TemplateRendererInterface;
use Opus\View\Html;
use ReflectionClass;
use ReflectionNamedType;

/*
 * OPUS_REFBOOK:
 *   domain: CONTROLLER
 *   role: Class ControllerDispatcher belongs to the CONTROLLER Opus framework domain.
 *   contract:
 *     - keeps responsibility limited to the CONTROLLER domain
 *     - exposes explicit behavior for the RefBook extractor
 *     - must not rely on silent fallback behavior
 *   examples:
 *     - controller-overview
 *   diagrams:
 *     - controller-runtime
 * END_OPUS_REFBOOK
 */
/**
 * PUBLIC LEGACY-ALIGNED DISPATCHER
 *
 * Role:
 *   Instantiate controllers and execute matched actions.
 *
 * Responsibility:
 *   Convert controller results into HTTP responses through official renderers.
 *
 * Legacy alignment:
 *   Lives in `Opus\Controller`, matching the original framework domain.
 *
 * Contract:
 *   Dispatcher does not route, does not authorize, does not read content and
 *   does not render templates directly.
 *
 * Since:
 *   P112D4C
 */
final class ControllerDispatcher
{
    public function __construct(
        private readonly ApplicationPaths $paths,
        private readonly TemplateRendererInterface $templateRenderer,
        private readonly HtmlRenderer $htmlRenderer
    ) {
    }

    public function dispatch(Request $request, RouteMatch $match): Response
    {
        $controller = $this->createController($match->controllerClass);

        if (!is_callable([$controller, $match->action])) {
            throw ControllerException::because('OPUS_CONTROLLER_ACTION_MISSING', $match->controllerClass . '::' . $match->action);
        }

        $result = $controller->{$match->action}($request, $match->params);

        if ($result instanceof Response) {
            return $result;
        }

        if ($result instanceof Html) {
            return $result->toResponse($this->templateRenderer);
        }

        if ($result instanceof ViewModel) {
            return $this->htmlRenderer->render($result);
        }

        throw ControllerException::because('OPUS_CONTROLLER_RESULT_INVALID', $match->controllerClass . '::' . $match->action);
    }

    private function createController(string $controllerClass): object
    {
        if (!class_exists($controllerClass)) {
            throw ControllerException::because('OPUS_CONTROLLER_CLASS_MISSING', $controllerClass);
        }

        $reflection = new ReflectionClass($controllerClass);
        $constructor = $reflection->getConstructor();

        if ($constructor === null || $constructor->getNumberOfParameters() === 0) {
            return $reflection->newInstance();
        }

        $parameters = $constructor->getParameters();

        if (count($parameters) !== 2) {
            throw ControllerException::because('OPUS_CONTROLLER_CONSTRUCTOR_CONTRACT_INVALID', $controllerClass);
        }

        $firstType = $parameters[0]->getType();
        $secondType = $parameters[1]->getType();

        if (!$firstType instanceof ReflectionNamedType || $firstType->getName() !== ApplicationPaths::class) {
            throw ControllerException::because('OPUS_CONTROLLER_CONSTRUCTOR_PATHS_INVALID', $controllerClass);
        }

        if (!$secondType instanceof ReflectionNamedType || $secondType->getName() !== TemplateRendererInterface::class) {
            throw ControllerException::because('OPUS_CONTROLLER_CONSTRUCTOR_RENDERER_INVALID', $controllerClass);
        }

        return $reflection->newInstance($this->paths, $this->templateRenderer);
    }
}
