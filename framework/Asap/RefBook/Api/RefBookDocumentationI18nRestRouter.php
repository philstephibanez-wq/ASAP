 ...
 * ASAP_REFBOOK:
 *   domain: REFBOOK
 *   role: Class RefBookDocumentationI18nRestRouter belongs to the REFBOOK ASAP framework domain.
 *   contract:
...
final class RefBookDocumentationI18nRestRouter
{
    ...
    public function __construct(
        ...
        if (!is_callable($this->snapshotProvider)) {
            throw new \InvalidArgumentException('ASAP_REFBOOK_DOC_SNAPSHOT_PROVIDER_NOT_CALLABLE');
        }
    }
    ...
    public function handle(string $method, string $requestUri): array
    {
        if (strtoupper($method) !== 'GET') {
            return $this->response(405, ['ok' => false, 'error' => 'ASAP_REFBOOK_DOC_METHOD_NOT_ALLOWED']);
        }

        $path = parse_url($requestUri, PHP_URL_PATH);
        if (!is_string($path)) {
            return $this->response(400, ['ok' => false, 'error' => 'ASAP_REFBOOK_DOC_PATH_INVALID']);
        }

        if (!preg_match('~^/api/refbook/(?P<lang>[a-z]{2})/(?P<resource>snapshot|symbols)(?:/(?P<id>[A-Za-z0-9_.-]+))?$~', $path, $matches)) {
            return $this->response(404, ['ok' => false, 'error' => 'ASAP_REFBOOK_DOC_ROUTE_NOT_FOUND']);
        }
        ...
        try {
            ...
            if (!is_array($snapshot)) {
                return $this->response(500, ['ok' => false, 'error' => 'ASAP_REFBOOK_DOC_SNAPSHOT_INVALID']);
            }
            ...
            if (($matches['id'] ?? '') !== '') {
                ...
                if ($symbol === null) {
                    return $this->response(404, ['ok' => false, 'error' => 'ASAP_REFBOOK_DOC_SYMBOL_NOT_FOUND', 'id' => $matches['id']]);
                }
