 ...
 * ASAP_REFBOOK:
 *   domain: DATABASE
 *   role: Class DatabaseConnectionConfig belongs to the DATABASE ASAP framework domain.
 *   contract:
...
final class DatabaseConnectionConfig
{
    ...
    public function __construct(
        ...
        if ($this->dsn !== null && trim($this->dsn) === '') {
            throw DatabaseException::because('ASAP_DATABASE_DSN_EMPTY');
        }
    }
    ...
    public function parameter(string $name): string
    {
        if (!array_key_exists($name, $this->parameters)) {
            throw DatabaseException::because('ASAP_DATABASE_PARAMETER_MISSING', $name);
        }

        $value = trim((string) $this->parameters[$name]);

        if ($value === '') {
            throw DatabaseException::because('ASAP_DATABASE_PARAMETER_EMPTY', $name);
        }
