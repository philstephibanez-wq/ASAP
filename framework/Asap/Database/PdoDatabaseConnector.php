 ...
 * ASAP_REFBOOK:
 *   domain: DATABASE
 *   role: Class PdoDatabaseConnector belongs to the DATABASE ASAP framework domain.
 *   contract:
 ...
 *
 * Role:
 *   Create explicit PDO-backed ASAP Database objects.
 *
 * Contract:
...
final class PdoDatabaseConnector
{
    ...
    public function connect(DatabaseConnectionConfig $config): Database
    {
        ...
        if (!in_array($driver, $available, true)) {
            throw DatabaseException::because('ASAP_DATABASE_PDO_DRIVER_UNAVAILABLE', $driver);
        }
        ...
        try {
            ...
            throw DatabaseException::because('ASAP_DATABASE_CONNECTION_FAILED', $exception->getMessage());
        }
