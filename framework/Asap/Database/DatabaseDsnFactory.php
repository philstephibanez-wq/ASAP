 ...
 * ASAP_REFBOOK:
 *   domain: DATABASE
 *   role: Class DatabaseDsnFactory belongs to the DATABASE ASAP framework domain.
 *   contract:
...
final class DatabaseDsnFactory
{
    public function build(DatabaseConnectionConfig $config): string
    {
        ...
        return match ($config->normalizedProvider()) {
            ...
            default => throw DatabaseException::because('ASAP_DATABASE_PROVIDER_UNSUPPORTED', $config->provider),
        };
    }
